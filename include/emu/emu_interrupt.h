 ...
 *
 * This file defines the abstract interrupt controller interface using a vtable
 * pattern. Architecture-specific implementations (NVIC for ARM, PLIC for
 * RISC-V) provide concrete interrupt handling.
 */
...
struct EmuInterruptVTable {
   ...
   */
  uint32_t (*read_reg)(EmuInterruptController *ic, uint32_t offset,
                       uint8_t size);
   ...
   * @param size      Access size
   */
  void (*write_reg)(EmuInterruptController *ic, uint32_t offset, uint32_t value,
                    uint8_t size);
...
#define EMU_IC_INIT(ic, vtbl, arch_type, state)                                \
  do {                                                                         \
    (ic)->vtable = (vtbl);                                                     \
    (ic)->arch = (arch_type);                                                  \
    (ic)->arch_state = (state);                                                \
...
static inline void emu_ic_set_priority(EmuInterruptController *ic, int irq,
                                       int priority) {
  if (ic && ic->vtable) {
...
static inline int emu_ic_get_priority(const EmuInterruptController *ic,
                                      int irq) {
  return (ic && ic->vtable) ? ic->vtable->get_priority(ic, irq) : 0;
}

static inline int emu_ic_get_pending_irq(EmuInterruptController *ic,
                                         int current_priority) {
  return (ic && ic->vtable) ? ic->vtable->get_pending_irq(ic, current_priority)
                            : -1;
}
