 ...
 *
 * This file defines the abstract instruction decoder interface using a vtable pattern.
 * Architecture-specific implementations provide concrete decoding functions.
 */
...
struct EmuDecoderVTable {
     ...
     */
    int (*decode)(EmuDecoder *dec, const uint8_t *mem, uint64_t pc, EmuDecodedInsn *insn);
     ...
     */
    int (*disassemble)(EmuDecoder *dec, const EmuDecodedInsn *insn, char *buf, size_t size);
...
#define EMU_DECODER_INIT(dec, vtbl, arch_type, state) do { \
    (dec)->vtable = (vtbl);                                 \
    (dec)->arch = (arch_type);                              \
...
static inline int emu_decode(EmuDecoder *dec, const uint8_t *mem, uint64_t pc, EmuDecodedInsn *insn) {
    return (dec && dec->vtable) ? dec->vtable->decode(dec, mem, pc, insn) : EMU_ERR_NOT_INITIALIZED;
}

static inline int emu_disassemble(EmuDecoder *dec, const EmuDecodedInsn *insn, char *buf, size_t size) {
    return (dec && dec->vtable && dec->vtable->disassemble)
           ? dec->vtable->disassemble(dec, insn, buf, size) : EMU_ERR_NOT_SUPPORTED;
}
...
static inline void emu_decoder_init_insn(EmuDecoder *dec, EmuDecodedInsn *insn) {
    if (dec && dec->vtable && dec->vtable->init_insn) {
        dec->vtable->init_insn(dec, insn);
    }
}

static inline void emu_decoder_free_insn(EmuDecoder *dec, EmuDecodedInsn *insn) {
