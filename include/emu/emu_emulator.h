...
struct EmuEmulatorVTable {
     ...
     * @return          EMU_OK or error code
     */
    int (*load)(EmuEmulator *emu, uint64_t addr, const uint8_t *data, uint64_t size);
     ...
     * @return          EMU_OK or error code
     */
    int (*add_peripheral)(EmuEmulator *emu, EmuPeripheral *periph,
 ...
 */
EmuEmulator* emu_emulator_create(EmuArchType arch, const EmuEmulatorConfig *config);
...
#define EMU_EMULATOR_INIT(emu, vtbl, arch_type, state) do { \
    (emu)->vtable = (vtbl);                                  \
    (emu)->arch = (arch_type);                               \
...
static inline int emu_add_peripheral(EmuEmulator *emu, EmuPeripheral *periph,
    ...
    return (emu && emu->vtable && emu->vtable->add_peripheral)
           ? emu->vtable->add_peripheral(emu, periph, base, size) : EMU_ERR_NOT_INITIALIZED;
}
