 ...
 *
 * This file defines the abstract instruction executor interface using a vtable pattern.
 * Architecture-specific implementations provide concrete execution functions.
 */
...
struct EmuMemoryCallbacks {
     ...
     * @param fault     Set to true if fault occurred
     */
    void (*write)(void *ctx, uint64_t addr, uint64_t value, uint8_t size, bool *fault);
...
#define EMU_EXECUTOR_INIT(exec, vtbl, arch_type, state) do { \
    (exec)->vtable = (vtbl);                                  \
    (exec)->arch = (arch_type);                               \
...
static inline void emu_executor_reset(EmuExecutor *exec, uint64_t reset_vector) {
    if (exec && exec->vtable) {
        exec->vtable->reset(exec, reset_vector);
    }
}

static inline void emu_executor_set_memory(EmuExecutor *exec, const EmuMemoryCallbacks *mem) {
...
static inline EmuDecoder* emu_executor_get_decoder(EmuExecutor *exec) {
    return (exec && exec->vtable && exec->vtable->get_decoder)
           ? exec->vtable->get_decoder(exec) : NULL;
}

static inline int emu_executor_exec_insn(EmuExecutor *exec, const EmuDecodedInsn *insn) {
    return (exec && exec->vtable) ? exec->vtable->exec_insn(exec, insn) : EMU_ERR_NOT_INITIALIZED;
}

static inline int emu_executor_step(EmuExecutor *exec) {
    return (exec && exec->vtable) ? exec->vtable->step(exec) : EMU_ERR_NOT_INITIALIZED;
}

static inline int64_t emu_executor_run(EmuExecutor *exec, uint64_t max_cycles) {
    return (exec && exec->vtable) ? exec->vtable->run(exec, max_cycles) : EMU_ERR_NOT_INITIALIZED;
}
