...
static int save_fpu_context(Executor *exec, uint32_t frameptr, bool *fault) {
  ...
  for (int i = 0; i < 16; i++) {
    mem_write(exec, frameptr + 32 + (uint32_t)(i * 4), cpu->s[i], ACCESS_WORD,
              fault);
    if (*fault)
...
static int restore_fpu_context(Executor *exec, uint32_t frameptr, bool *fault) {
  ...
  for (int i = 0; i < 16; i++) {
    cpu->s[i] =
        mem_read(exec, frameptr + 32 + (uint32_t)(i * 4), ACCESS_WORD, fault);
    if (*fault)
      return ARMV8M_ERR_BUS_FAULT;
  }
...
static int validate_exc_return(const Executor *exec, uint32_t exc_return) {
  ...
  if (exec->has_trustzone) {
    ...
    if (exec->cpu.security == SECURITY_NONSECURE && exc_secure &&
        stack_secure) {
...
int armv8m_exception_entry(Executor *exec, int exception) {
  ...
  EMU_LOG_INFO(EMU_LOG_CAT_NVIC,
               "Exception entry: exc=%d from PC=0x%08X mode=%s", exception,
               cpu->r[ARMV8M_REG_PC],
  ...
  uint32_t sp;
  bool use_psp =
      (cpu->mode == MODE_THREAD) && (cpu->control & ARMV8M_CONTROL_SPSEL);
  ...
  uint32_t frame_size =
      use_extended_frame ? EXCEPTION_FRAME_EXTENDED : EXCEPTION_FRAME_BASIC;
...
int armv8m_exception_return(Executor *exec, uint32_t exc_return) {
  ...
  EMU_LOG_DEBUG(EMU_LOG_CAT_NVIC,
                "Exception return: EXC_RETURN=0x%08X current_exc=%d",
  ...
  bool return_to_psp = (exc_return & EXC_RETURN_SPSEL) != 0;
  bool extended_frame =
      (exc_return & EXC_RETURN_FTYPE) == 0; /* FTYPE=0 means extended */
  ...
  uint32_t frame_size =
      extended_frame ? EXCEPTION_FRAME_EXTENDED : EXCEPTION_FRAME_BASIC;
