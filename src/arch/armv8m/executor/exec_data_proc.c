 ...
 */
static uint32_t sub_with_flags(uint32_t a, uint32_t b, bool *carry_out,
                               bool *overflow) {
...
static uint32_t apply_shift(uint32_t value, ShiftType type, uint8_t amount,
  ...
  *carry_out = carry_in;
  ...
  if (amount == 0 && type != SHIFT_RRX && type != SHIFT_LSR &&
      type != SHIFT_ASR) {
 ...
 */
static int exec_dp_operation(Executor *exec, DataProcOp op, uint8_t rd,
                             uint32_t rn_val, uint32_t op2, bool set_flags,
                             bool shift_carry) {
  ...
  if (write_result && rd != ARMV8M_REG_NONE) {
    EMU_LOG_TRACE(EMU_LOG_CAT_EXECUTOR,
                  "DP op=%d R%d=0x%X op2=0x%X -> R%d=0x%X%s", op, rd, rn_val,
                  op2, rd, result, set_flags ? " [S]" : "");
...
int exec_data_proc_imm(Executor *exec, const DecodedInsn *insn) {
  ...
  return exec_dp_operation(exec, (DataProcOp)insn->op, insn->rd, rn_val, imm,
...
int exec_data_proc_reg(Executor *exec, const DecodedInsn *insn) {
  ...
  if (insn->op == DP_ROR && insn->shift_amount >= 0x10 &&
      insn->shift_amount <= 0x13) {
    uint32_t result;
    switch (insn->shift_amount) {
    case 0x10: /* REV - byte reverse word */
      result = ((rm_val & 0xFF) << 24) | ((rm_val & 0xFF00) << 8) |
               ((rm_val & 0xFF0000) >> 8) | ((rm_val & 0xFF000000) >> 24);
      break;
    case 0x11: /* REV16 - byte reverse packed halfwords */
      result = ((rm_val & 0x00FF) << 8) | ((rm_val & 0xFF00) >> 8) |
               ((rm_val & 0x00FF0000) << 8) | ((rm_val & 0xFF000000) >> 8);
      break;
    case 0x12: /* RBIT - bit reverse */
      result = reverse_bits(rm_val);
      break;
    case 0x13: /* REVSH - byte reverse signed halfword */
      result = (uint32_t)(int32_t)(int16_t)(((rm_val & 0xFF) << 8) |
                                            ((rm_val & 0xFF00) >> 8));
      break;
    default:
      result = 0;
  ...
  return exec_dp_operation(exec, (DataProcOp)insn->op, insn->rd, rn_val, rm_val,
                           set_flags, shift_carry);
}
...
int exec_data_proc_shifted(Executor *exec, const DecodedInsn *insn) {
  ...
  return exec_dp_operation(exec, (DataProcOp)insn->op, insn->rd, rn_val,
                           shifted, insn->set_flags, shift_carry);
}
...
int exec_multiply(Executor *exec, const DecodedInsn *insn) {
  ...
  case MUL_SMLAL: {
    ...
    result64 =
        (uint64_t)(acc + (int64_t)(int32_t)rn_val * (int64_t)(int32_t)rm_val);
    is_long = true;
    break;
  }
  ...
  case MUL_UMLAL: {
    uint64_t acc =
        ((uint64_t)get_reg(exec, insn->rt) << 32) | get_reg(exec, insn->rd);
  ...
  case MUL_SMMUL:
    result =
        (uint32_t)(((int64_t)(int32_t)rn_val * (int64_t)(int32_t)rm_val) >> 32);
    break;

  case MUL_SMMULR:
    result = (uint32_t)(((int64_t)(int32_t)rn_val * (int64_t)(int32_t)rm_val +
                         0x80000000LL) >>
                        32);
    break;
  ...
  case MUL_SMMLA:
    result =
        (uint32_t)((((int64_t)(int32_t)ra_val << 32) +
                    ((int64_t)(int32_t)rn_val * (int64_t)(int32_t)rm_val)) >>
                   32);
    break;
...
int exec_divide(Executor *exec, const DecodedInsn *insn) {
  ...
  EMU_LOG_TRACE(EMU_LOG_CAT_EXECUTOR, "%s 0x%X / 0x%X -> R%d=0x%X",
                insn->is_signed ? "SDIV" : "UDIV", rn_val, rm_val, insn->rd,
                result);
  set_reg(exec, insn->rd, result);
  return ARMV8M_OK;
}
...
int exec_saturate(Executor *exec, const DecodedInsn *insn) {
  ...
  EMU_LOG_TRACE(EMU_LOG_CAT_EXECUTOR, "%s #%d, R%d -> R%d=0x%X",
                insn->is_signed ? "SSAT" : "USAT", sat_width, insn->rn,
                insn->rd, result);
  set_reg(exec, insn->rd, result);
  return ARMV8M_OK;
}
...
static void set_ge_flags_8(CPUState *cpu, int res0, int res1, int res2,
                           int res3) {
  uint32_t ge = 0;

  /* GE flags are set based on the 8-bit truncated result interpreted as signed
   */
  if ((int8_t)(res0 & 0xFF) >= 0)
    ge |= 0x1;
  if ((int8_t)(res1 & 0xFF) >= 0)
    ge |= 0x2;
  if ((int8_t)(res2 & 0xFF) >= 0)
    ge |= 0x4;
  if ((int8_t)(res3 & 0xFF) >= 0)
    ge |= 0x8;
