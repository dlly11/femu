...
inline uint32_t integ_mem_read(void *ctx, uint32_t addr, uint8_t size, bool *fault) {
    (void)ctx;
    ...
    switch (size) {
        case 1: return test_memory[addr];
        case 2: return test_memory[addr] | ((uint32_t)test_memory[addr + 1] << 8);
        case 4: return test_memory[addr] | ((uint32_t)test_memory[addr + 1] << 8) |
...
inline void integ_mem_write(void *ctx, uint32_t addr, uint32_t value, uint8_t size, bool *fault) {
    (void)ctx;
    if (fault) *fault = false;
...
inline const uint8_t* integ_mem_get_ptr(void *ctx, uint32_t addr, uint32_t size) {
    (void)ctx;
    if (addr + size > MEMORY_SIZE) {
        return NULL;
    }
