...
inline uint32_t mock_mem_read(void *ctx, uint32_t addr, uint8_t size, bool *fault) {
    (void)ctx;
    ...
    switch (size) {
        ...
        case 2: return mock_memory[addr] | ((uint32_t)mock_memory[addr + 1] << 8);
        case 4: return mock_memory[addr] | ((uint32_t)mock_memory[addr + 1] << 8) |
                       ((uint32_t)mock_memory[addr + 2] << 16) | ((uint32_t)mock_memory[addr + 3] << 24);
    }
    return 0;
}

inline void mock_mem_write(void *ctx, uint32_t addr, uint32_t value, uint8_t size, bool *fault) {
    (void)ctx;
    if (fault) *fault = false;
...
inline const uint8_t* mock_mem_get_ptr(void *ctx, uint32_t addr, uint32_t size) {
    (void)ctx;
    if (addr + size > sizeof(mock_memory)) {
        return NULL;
    }
...
inline void expect_decode_init() {
    mock().expectOneCall("armv8m_decode_init");
}

inline void expect_decode(uint32_t pc, int return_value = 2) {
    mock().expectOneCall("armv8m_decode")
          .withParameter("pc", pc)
          .withOutputParameterReturning("insn", &mock_decoded_insn, sizeof(mock_decoded_insn))
