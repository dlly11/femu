...
TEST(DSP_Parallel, SEL_SelectBytes)
{
     ...
     */
    exec.cpu.xpsr = (exec.cpu.xpsr & ~ARMV8M_XPSR_GE_MASK) | (0x5 << ARMV8M_XPSR_GE_SHIFT);
    ...
    CHECK_EQUAL(ARMV8M_OK, armv8m_exec_insn(&exec, &insn));
    /* Byte 0 from Rn (DD), byte 1 from Rm (33), byte 2 from Rn (BB), byte 3 from Rm (11)
     * Result: 0x11BB33DD */
...
TEST(DSP_Parallel, SEL_AllFromRn)
{
    ...
    exec.cpu.xpsr = (exec.cpu.xpsr & ~ARMV8M_XPSR_GE_MASK) | (0xF << ARMV8M_XPSR_GE_SHIFT);
...
TEST(DSP_GEFlags, SADD8_MixedResults)
{
    ...
    CHECK_EQUAL(ARMV8M_OK, armv8m_exec_insn(&exec, &insn));
    /* Byte 0: -1 + 1 = 0 (>= 0), Byte 1: 1 + 1 = 2, Byte 2: 1 + 1 = 2, Byte 3: 127 + 1 = -128 (< 0) */
