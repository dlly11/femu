...
TEST(ExceptionCoverage, ExceptionReturnToPsp) {
  ...
  int result =
      armv8m_exception_return(&exec, 0xFFFFFFFD); /* Return to Thread, PSP */
  CHECK_EQUAL(ARMV8M_OK, result);
...
TEST(ExceptionCoverage, ExceptionEntryUnalignedSp) {
  ...
  CHECK_EQUAL(ARMV8M_OK, result);
  /* SP should be aligned down to 0x100 - 32 = 0xE0 (rounded down, then frame
   * subtracted) */
}
...
TEST(ExceptionCoverage, ExceptionReturnAlignedStack) {
  ...
  int result =
      armv8m_exception_return(&exec, 0xFFFFFFF9); /* Return to Thread, MSP */
  CHECK_EQUAL(ARMV8M_OK, result);
...
TEST(ExceptionCoverage, ExceptionReturnToHandler) {
  ...
  int result =
      armv8m_exception_return(&exec, 0xFFFFFFF1); /* Return to Handler, MSP */
  CHECK_EQUAL(ARMV8M_OK, result);
  CHECK_EQUAL(MODE_HANDLER, exec.cpu.mode);
}
