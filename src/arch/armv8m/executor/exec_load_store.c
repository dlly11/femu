 ...
 */
static uint32_t mem_read(Executor *exec, uint32_t addr, AccessSize size,
                         bool *fault) {
  *fault = false;
  ...
  if (*fault) {
    EMU_LOG_DEBUG(EMU_LOG_CAT_MEMORY, "Read fault at 0x%08X size=%d", addr,
                  (int)size);
  } else {
    EMU_LOG_TRACE(EMU_LOG_CAT_MEMORY, "Read 0x%08X size=%d -> 0x%X", addr,
                  (int)size, value);
  }
  return value;
}
...
static void mem_write(Executor *exec, uint32_t addr, uint32_t value,
  ...
  *fault = false;
  ...
  if (*fault) {
    EMU_LOG_DEBUG(EMU_LOG_CAT_MEMORY,
                  "Write fault at 0x%08X size=%d value=0x%X", addr, (int)size,
                  value);
  } else {
    EMU_LOG_TRACE(EMU_LOG_CAT_MEMORY, "Write 0x%08X size=%d <- 0x%X", addr,
                  (int)size, value);
  }
}
