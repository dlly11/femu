 ...
 *
 * Implements MRS, MSR, CPS, barriers (DMB, DSB, ISB), hints (NOP, WFI, WFE,
 * SEV).
 */
...
/* Note: SAU registers are memory-mapped (0xE000EDD0-0xE000EDE0), not accessible
 * via MRS/MSR. SAU register access is handled through the memory subsystem. */
...
int exec_mrs(Executor *exec, const DecodedInsn *insn) {
  ...
  EMU_LOG_TRACE(EMU_LOG_CAT_EXECUTOR, "MRS R%d, %s -> 0x%08X", insn->rd,
                sysreg_name(sysreg), value);
...
int exec_msr(Executor *exec, const DecodedInsn *insn) {
  ...
  case SYSREG_MSP:
    if (privileged) {
      ...
      if (cpu->mode == MODE_HANDLER || !(cpu->control & ARMV8M_CONTROL_SPSEL)) {
  ...
  case SYSREG_PSP:
    if (privileged) {
      ...
      if (cpu->mode == MODE_THREAD && (cpu->control & ARMV8M_CONTROL_SPSEL)) {
...
int exec_svc(Executor *exec, const DecodedInsn *insn) {
  EMU_LOG_INFO(EMU_LOG_CAT_EXECUTOR, "SVC #%d at 0x%08X", insn->imm & 0xFF,
               exec->cpu.r[ARMV8M_REG_PC]);
...
int exec_tt(Executor *exec, const DecodedInsn *insn) {
  ...
  if (is_alt) {
    ...
    query_state = (exec->cpu.security == SECURITY_SECURE) ? SECURITY_NONSECURE
                                                          : SECURITY_SECURE;
  } else {
