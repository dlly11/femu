...
static const EmuRegisterDesc armv8m_registers[] = {
    ...
    { "lr",     14, 4, 14 },
    { "pc",     15, 4, 15 },
    { "xpsr",   16, 4, 25 },  /* GDB uses 25 for xPSR */
};
...
static uint64_t armv8m_cpu_get_special_reg(const EmuCPU *cpu, int reg_id)
{
    ...
    switch (reg_id) {
        ...
        default:
            /* FPU S registers */
            if (reg_id >= ARMV8M_CPU_SREG_S0 &&
                reg_id < ARMV8M_CPU_SREG_S0 + 32) {
...
static void armv8m_cpu_set_special_reg(EmuCPU *cpu, int reg_id, uint64_t value)
{
    if (!cpu) {
        return;
    }
