...
TEST_GROUP(InterruptEnable) {
  ...
  void setup() { armv8m_nvic_init(&nvic, 64); }
...
TEST_GROUP(Priority) {
  ...
  void setup() { armv8m_nvic_init(&nvic, 32); }
...
TEST(ARMCompliance, SHCSRReadMasksInternalBits) {
  ...
  /* Bits 27-31 (internal tracking: HardFault pending, PendSV, SysTick,
   * DebugMon, NMI) should be masked out */
  CHECK_EQUAL(0x07FFFFFFu, value);
}

TEST(ARMCompliance, SHCSRReadPreservesVisibleBits) {
  /* Set some visible SHCSR bits */
  nvic.shcsr = (1u << 0) | (1u << 7) |
               (1u << 16); /* MEMFAULTACT, SVCALLACT, MEMFAULTENA */
