...
static bool addr_in_region(const MPURegion *r, uint32_t addr) {
  ...
  uint32_t limit =
      r->rlar | 0x1F; /* Limit is inclusive, round up to 32-byte boundary */
 ...
 */
static bool check_permission(const MPURegion *r, bool is_write, bool is_fetch,
                             bool privileged) {
...
static bool check_single_addr(const MPU *mpu, int region, uint32_t addr,
  ...
  return check_permission(&mpu->regions[region], is_write, is_fetch,
                          privileged);
}
...
static bool check_spanning_access(const MPU *mpu, int region, int end_region,
  ...
  /* Check end address */
  if (!check_single_addr(mpu, end_region, end_addr, is_write, is_fetch,
                         privileged)) {
    return false;
  }
...
int armv8m_mpu_configure_region(MPU *mpu, int region, uint32_t base,
  ...
  uint32_t rbar = (base & MPU_RBAR_BASE_MASK) |
                  ...
                  ((uint32_t)ap << MPU_RBAR_AP_SHIFT) | (xn ? MPU_RBAR_XN : 0);
...
void armv8m_mpu_write(MPU *mpu, uint32_t offset, uint32_t value, uint8_t size) {
  ...
  case MPU_CTRL:
    mpu->ctrl =
        value & (MPU_CTRL_ENABLE | MPU_CTRL_HFNMIENA | MPU_CTRL_PRIVDEFENA);
...
uint8_t armv8m_mpu_get_attributes(const MPU *mpu, uint32_t addr,
  ...
  uint32_t attr_idx = (mpu->regions[region].rlar >> MPU_RLAR_ATTRINDX_SHIFT) &
                      MPU_RLAR_ATTRINDX_MASK;
