...
/* Common register indices for readability (use uint8_t to avoid CHECK_EQUAL
 * ambiguity) */
...
TEST_GROUP(DecoderInit) {
  ...
  void setup() { armv8m_decode_init(&insn); }
...
TEST(Thumb16Coverage, LsrImmZero) {
  ...
  CHECK_EQUAL(INSN_DATA_PROC_SHIFTED, insn.type);
  CHECK_EQUAL(DP_MOV,
              insn.op); /* Immediate shift uses MOV with shift applied */
...
TEST(Thumb32Coverage, LdrhW_LoadHalfword) {
  /*
   * LDRH.W R0, [R1, #0x80] - Load unsigned halfword with 12-bit immediate (T2
   * encoding)
   *
