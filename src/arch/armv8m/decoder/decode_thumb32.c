 ...
 */
/**
 * Decode the Thumb-2 data-processing (register) group (op1=0b11, op==1,
 * op2=010xxxx): register extends, register-controlled shifts, parallel
 * add/sub, misc DSP ops, and the shifted-register fallback.
 */
static int decode_t32_dp_register(uint16_t hw1, uint16_t hw2,
                                  DecodedInsn *out) {
...
/**
 * Decode the Thumb-2 op1=0b11 group: multiply/divide, load/store single,
 * plain-immediate data processing, branches, and the register data-proc
 * sub-group. Returns ARMV8M_ERR_UNDEFINED_INSN if nothing matches.
 */
static int decode_t32_op11(uint16_t hw1, uint16_t hw2, uint32_t pc,
                           DecodedInsn *out) {
  uint32_t op2 = EXTRACT(hw1, 4, 7);
  uint32_t op = BIT(hw2, 15);
  ...
  if (op == 0) {
    ...
    if ((op2 & 0x71) == 0x00 || (op2 & 0x67) == 0x01 || (op2 & 0x67) == 0x03 ||
        (op2 & 0x67) == 0x05 || (op2 & 0x71) == 0x20) {
    ...
    /* Load/store single (multiple patterns) */
    if ((op2 & 0x70) == 0x00 || (op2 & 0x70) == 0x40 || (op2 & 0x70) == 0x50) {
      return decode_load_store_single(hw1, hw2, out);
    }
    if ((op2 & 0x70) == 0x20) {
      return decode_t32_dp_register(hw1, hw2, out);
    }
  }
...
int decode_thumb32(uint16_t hw1, uint16_t hw2, uint32_t pc, DecodedInsn *out) {
  ...
  case 0x3: /* 0b11 */
    return decode_t32_op11(hw1, hw2, pc, out);
