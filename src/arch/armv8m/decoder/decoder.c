...
int armv8m_decode(const uint8_t *mem, uint32_t pc, DecodedInsn *insn) {
  ...
  if (armv8m_is_thumb32(hw1)) {
    ...
    if (result < 0) {
      EMU_LOG_DEBUG(EMU_LOG_CAT_DECODER,
                    "Decode error at PC=0x%08X: encoding=0x%08X result=%d", pc,
                    insn->encoding, result);
      return result;
    }
    return 4;
  }
  ...
  if (result < 0) {
    EMU_LOG_DEBUG(EMU_LOG_CAT_DECODER,
                  "Decode error at PC=0x%08X: encoding=0x%04X result=%d", pc,
                  insn->encoding, result);
    return result;
  }
  return 2;
...
const char *armv8m_insn_mnemonic(InstructionType type, uint8_t op) {
  static const char *dp_mnemonics[] = {
      "AND", "EOR", "LSL", "LSR", "ASR", "ADC", "SBC", "ROR", "TST", "RSB",
      "CMP", "CMN", "ORR", "MUL", "BIC", "MVN", "ADD", "SUB", "MOV", "ORN"};
...
int armv8m_disasm(const DecodedInsn *insn, char *buf, size_t buf_size) {
  ...
  case INSN_DATA_PROC_IMM:
    written =
        snprintf(buf, buf_size, "%s R%d, #%u", mnemonic, insn->rd, insn->imm);
    break;
  case INSN_DATA_PROC_REG:
    written = snprintf(buf, buf_size, "%s R%d, R%d, R%d", mnemonic, insn->rd,
                       insn->rn, insn->rm);
    break;
  case INSN_LOAD_IMM:
  case INSN_STORE_IMM:
    written = snprintf(buf, buf_size, "%s R%d, [R%d, #%u]", mnemonic, insn->rt,
                       insn->rn, insn->imm);
    break;
  case INSN_BRANCH:
  case INSN_BRANCH_LINK:
    written = snprintf(buf, buf_size, "%s %+d", mnemonic, insn->branch_offset);
    break;
  case INSN_BRANCH_EXCHANGE:
