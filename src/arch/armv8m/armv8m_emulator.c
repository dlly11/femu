 ...
 */
static uint32_t mem_read_callback(void *ctx, uint32_t addr, uint8_t size,
                                  bool *fault) {
 ...
 */
static void mem_write_callback(void *ctx, uint32_t addr, uint32_t value,
 ...
 */
static const uint8_t *mem_get_ptr_callback(void *ctx, uint32_t addr,
                                           uint32_t size) {
  Emulator *emu = (Emulator *)ctx;
...
static int nvic_get_pending_callback(void *ctx) {
  ...
  return armv8m_nvic_get_pending_exception(
      &emu->nvic, (uint8_t)emu->exec.cpu.basepri,
      (uint8_t)emu->exec.cpu.primask, (uint8_t)emu->exec.cpu.faultmask,
      current_pri);
}
...
static int nvic_get_priority_callback(void *ctx, int exc) {
  ...
  if (exc < 4) {
    ...
    switch (exc) {
    case ARMV8M_EXC_RESET:
      return -3;
    case ARMV8M_EXC_NMI:
      return -2;
    case ARMV8M_EXC_HARDFAULT:
      return -1;
    default:
      return 0;
    }
...
static bool mpu_check_callback(void *ctx, uint64_t addr, uint64_t size,
  ...
  bool in_hardfault_nmi =
      (emu->exec.cpu.current_exception == ARMV8M_EXC_HARDFAULT ||
       emu->exec.cpu.current_exception == ARMV8M_EXC_NMI);

  MPUFaultInfo fault_info;
  bool ok =
      armv8m_mpu_check(&emu->mpu, (uint32_t)addr, (uint32_t)size, is_write,
 ...
 */
static void periph_mmio_write(void *ctx, uint64_t offset, uint64_t value,
                              uint8_t size) {
  EmuPeripheral *periph = (EmuPeripheral *)ctx;
  if (periph && periph->vtable.write) {
    periph->vtable.write(periph->context, (uint32_t)offset, (uint32_t)value,
                         size);
  }
}
 ...
 */
static void nvic_mmio_write(void *ctx, uint64_t offset, uint64_t value,
                            uint8_t size) {
  Emulator *emu = (Emulator *)ctx;
 ...
 */
static void scb_mmio_write(void *ctx, uint64_t offset, uint64_t value,
                           uint8_t size) {
  Emulator *emu = (Emulator *)ctx;
 ...
 */
static void mpu_mmio_write(void *ctx, uint64_t offset, uint64_t value,
                           uint8_t size) {
  Emulator *emu = (Emulator *)ctx;
...
int armv8m_emu_init(Emulator *emu, const EmulatorConfig *config) {
  ...
  EMU_LOG_INFO(
      EMU_LOG_CAT_EMULATOR,
      "Initializing ARMv8-M emulator (FPU=%d, DSP=%d, TZ=%d, MPU=%d, IRQs=%d)",
      cfg.has_fpu, cfg.has_dsp, cfg.has_trustzone, cfg.num_mpu_regions,
      cfg.num_irqs);
  ...
  emu_mem_add_mmio(&emu->mem, 0xE000E100, 0x0C00, emu, nvic_mmio_read,
                   nvic_mmio_write);

  /* SCB registers: 0xE000ED00 - 0xE000ED8F */
  emu_mem_add_mmio(&emu->mem, 0xE000ED00, 0x0090, emu, scb_mmio_read,
                   scb_mmio_write);
...
void armv8m_emu_reset(Emulator *emu) {
  ...
  EMU_LOG_INFO(EMU_LOG_CAT_EMULATOR, "Resetting emulator (VTOR=0x%08X)",
               emu->exec.vtor);
...
int armv8m_emu_add_flash(Emulator *emu, uint32_t base, uint32_t size) {
  ...
  EMU_LOG_INFO(EMU_LOG_CAT_MEMORY,
               "Adding flash region: base=0x%08X, size=0x%X (%u KB)", base,
               size, size / 1024);
  ...
  /* In a real system flash would be read-only, but for emulation we need to
   * write it first */
...
int armv8m_emu_add_ram(Emulator *emu, uint32_t base, uint32_t size) {
  ...
  EMU_LOG_INFO(EMU_LOG_CAT_MEMORY,
               "Adding RAM region: base=0x%08X, size=0x%X (%u KB)", base, size,
               size / 1024);
...
int64_t armv8m_emu_run(Emulator *emu, uint64_t max_cycles) {
  ...
  EMU_LOG_DEBUG(EMU_LOG_CAT_EMULATOR,
                "Starting execution at PC=0x%08X (max_cycles=%lu)",
  ...
  uint64_t target_cycles =
      max_cycles > 0 ? start_cycles + max_cycles : UINT64_MAX;
  ...
  EMU_LOG_DEBUG(EMU_LOG_CAT_EMULATOR,
                "Execution stopped: state=%d, cycles=%lu, PC=0x%08X",
                emu->state, (unsigned long)executed,
                emu->exec.cpu.r[ARMV8M_REG_PC]);
...
uint32_t armv8m_emu_get_reg(const Emulator *emu, int reg) {
  if (!emu || reg < 0 || reg >= ARMV8M_NUM_REGS)
    return 0;
...
uint32_t armv8m_emu_read_mem(const Emulator *emu, uint32_t addr, uint8_t size,
  ...
  return (uint32_t)emu_mem_read((EmuMemorySystem *)&emu->mem, addr, size, true,
                                fault);
}
...
int armv8m_emu_add_watchpoint(Emulator *emu, uint32_t addr, uint32_t size,
  ...
  for (int i = 0; i < emu->num_watchpoints; i++) {
    if (emu->watchpoints[i].active && emu->watchpoints[i].addr == addr &&
        emu->watchpoints[i].size == size && emu->watchpoints[i].type == type) {
      return ARMV8M_OK; /* Already exists */
    }
  }
  ...
  EMU_LOG_DEBUG(EMU_LOG_CAT_EMULATOR,
                "Added watchpoint: addr=0x%08X size=%u type=%d", addr, size,
                type);
...
int armv8m_emu_remove_watchpoint(Emulator *emu, uint32_t addr, uint32_t size,
  ...
  for (int i = 0; i < EMU_MAX_WATCHPOINTS; i++) {
    if (emu->watchpoints[i].active && emu->watchpoints[i].addr == addr &&
      ...
      emu->num_watchpoints--;
      EMU_LOG_DEBUG(EMU_LOG_CAT_EMULATOR,
                    "Removed watchpoint: addr=0x%08X size=%u type=%d", addr,
                    size, type);
      return ARMV8M_OK;
    }
  }
...
int armv8m_emu_add_peripheral(Emulator *emu, EmuPeripheral *periph,
  ...
  if (emu->num_peripherals >= EMU_MAX_PERIPHERALS) {
    EMU_LOG_ERROR(EMU_LOG_CAT_PERIPHERAL,
                  "Maximum number of peripherals reached");
    return ARMV8M_ERR_INVALID_PARAM;
  }

  EMU_LOG_INFO(EMU_LOG_CAT_PERIPHERAL,
               "Adding peripheral '%s' (type=%s) at 0x%08X-0x%08X",
               periph->name ? periph->name : "unnamed",
               periph->type ? periph->type : "unknown", base, base + size - 1);
  ...
  /* Add MMIO region */
  int result = emu_mem_add_mmio(&emu->mem, base, size, periph, periph_mmio_read,
                                periph_mmio_write);
  if (result != ARMV8M_OK) {
    EMU_LOG_ERROR(EMU_LOG_CAT_PERIPHERAL,
                  "Failed to add MMIO region for peripheral");
    return result;
  }
...
uint32_t armv8m_emu_get_special_reg(const Emulator *emu, int reg) {
  ...
  switch (reg) {
  case ARMV8M_SYSREG_PRIMASK:
    return emu->exec.cpu.primask;
  case ARMV8M_SYSREG_BASEPRI:
    return emu->exec.cpu.basepri;
  case ARMV8M_SYSREG_FAULTMASK:
    return emu->exec.cpu.faultmask;
  case ARMV8M_SYSREG_CONTROL:
    return emu->exec.cpu.control;
  case ARMV8M_SYSREG_MSP:
    return emu->exec.cpu.sp_main;
  case ARMV8M_SYSREG_PSP:
    return emu->exec.cpu.sp_process;
  case ARMV8M_SYSREG_MSPLIM:
    return emu->exec.cpu.msplim;
  case ARMV8M_SYSREG_PSPLIM:
    return emu->exec.cpu.psplim;
  default:
    return 0;
  }
}
...
void armv8m_emu_set_special_reg(Emulator *emu, int reg, uint32_t value) {
  ...
  switch (reg) {
  case ARMV8M_SYSREG_PRIMASK:
    emu->exec.cpu.primask = value;
    break;
  case ARMV8M_SYSREG_BASEPRI:
    emu->exec.cpu.basepri = value;
    break;
  case ARMV8M_SYSREG_FAULTMASK:
    emu->exec.cpu.faultmask = value;
    break;
  case ARMV8M_SYSREG_CONTROL:
    emu->exec.cpu.control = value;
    break;
  case ARMV8M_SYSREG_MSP:
    emu->exec.cpu.sp_main = value;
    break;
  case ARMV8M_SYSREG_PSP:
    emu->exec.cpu.sp_process = value;
    break;
  case ARMV8M_SYSREG_MSPLIM:
    emu->exec.cpu.msplim = value;
    break;
  case ARMV8M_SYSREG_PSPLIM:
    emu->exec.cpu.psplim = value;
    break;
  default:
    break;
  }
