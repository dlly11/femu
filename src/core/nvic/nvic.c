 ...
 */
static inline int irq_bit(int irq)
{
    return irq & 31;
}
 ...
 */
static inline int irq_reg(int irq)
{
    return irq >> 5;
}
 ...
 * PRIGROUP determines how the priority field is split between group and subpriority.
 * Only group priority is used for preemption; subpriority is for ordering within same group.
 */
...
void armv8m_scb_write(NVIC *nvic, uint32_t offset, uint32_t value, uint8_t size)
{
    ...
    case SCB_AIRCR:
        ...
        nvic->prigroup = (int)((value & AIRCR_PRIGROUP_MASK) >> AIRCR_PRIGROUP_SHIFT);
