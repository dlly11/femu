...
TEST_GROUP(EmulatorTests) {
  ...
  void setup() { memset(&emu, 0, sizeof(emu)); }

  void teardown() { armv8m_emu_destroy(&emu); }
};
...
TEST(EmulatorTests, BlockReadWrite) {
  ...
  uint32_t written =
      armv8m_emu_write_block(&emu, 0x20000000, write_data, sizeof(write_data));
  CHECK_EQUAL(sizeof(write_data), written);

  uint8_t read_data[5];
  uint32_t read =
      armv8m_emu_read_block(&emu, 0x20000000, read_data, sizeof(read_data));
...
TEST(EmulatorTests, WatchpointNullPointerHandling) {
  ...
  CHECK_EQUAL(ARMV8M_ERR_INVALID_PARAM,
              armv8m_emu_add_watchpoint(nullptr, 0, 4, WATCHPOINT_WRITE));
  CHECK_EQUAL(ARMV8M_ERR_INVALID_PARAM,
              armv8m_emu_remove_watchpoint(nullptr, 0, 4, WATCHPOINT_WRITE));
