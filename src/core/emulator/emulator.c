...
static uint32_t mem_read_callback(void *ctx, uint32_t addr, uint8_t size, bool *fault)
{
    ...
    /* Determine privilege level and fault context */
    ...
    return armv8m_mem_read(&emu->mem, addr, size, privileged, in_hardfault_nmi, fault);
}
...
static void mem_write_callback(void *ctx, uint32_t addr, uint32_t value, uint8_t size, bool *fault)
{
    ...
    armv8m_mem_write(&emu->mem, addr, value, size, privileged, in_hardfault_nmi, fault);
}
...
{
    Emulator *emu = (Emulator *)ctx;
    return armv8m_mem_get_ptr(&emu->mem, addr, size);
}
 ...
 */
static bool mpu_check_callback(void *ctx, uint32_t addr, uint32_t size,
                                bool is_write, bool privileged, bool in_hardfault_nmi)
{
    ...
    bool ok = armv8m_mpu_check(&emu->mpu, addr, size, is_write, false, privileged,
                                in_hardfault_nmi, &fault_info);
    return ok;
}
 ...
 */
static uint32_t periph_mmio_read(void *ctx, uint32_t offset, uint8_t size)
{
    Peripheral *periph = (Peripheral *)ctx;
    if (periph && periph->vtable.read) {
        return periph->vtable.read(periph->context, offset, size);
    }
    return 0;
}
 ...
 */
static void periph_mmio_write(void *ctx, uint32_t offset, uint32_t value, uint8_t size)
{
    Peripheral *periph = (Peripheral *)ctx;
    if (periph && periph->vtable.write) {
        periph->vtable.write(periph->context, offset, value, size);
    }
}
 ...
 */
static uint32_t nvic_mmio_read(void *ctx, uint32_t offset, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    return armv8m_nvic_read(&emu->nvic, offset, size);
}

/**
 * NVIC MMIO write callback.
 */
static void nvic_mmio_write(void *ctx, uint32_t offset, uint32_t value, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    armv8m_nvic_write(&emu->nvic, offset, value, size);
}

/**
 * SCB MMIO read callback (0xE000ED00).
 */
static uint32_t scb_mmio_read(void *ctx, uint32_t offset, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    return armv8m_scb_read(&emu->nvic, offset, size);
}

/**
 * SCB MMIO write callback.
 */
static void scb_mmio_write(void *ctx, uint32_t offset, uint32_t value, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    armv8m_scb_write(&emu->nvic, offset, value, size);
}

/**
 * MPU MMIO read callback (0xE000ED90).
 */
static uint32_t mpu_mmio_read(void *ctx, uint32_t offset, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    return armv8m_mpu_read(&emu->mpu, offset, size);
}

/**
 * MPU MMIO write callback.
 */
static void mpu_mmio_write(void *ctx, uint32_t offset, uint32_t value, uint8_t size)
{
    Emulator *emu = (Emulator *)ctx;
    armv8m_mpu_write(&emu->mpu, offset, value, size);
}
...
void armv8m_emu_reset(Emulator *emu)
{
    ...
    if (emu->flash_data) {
        const uint8_t *vt = armv8m_mem_get_ptr(&emu->mem, emu->exec.vtor, 8);
        if (vt) {
...
int armv8m_emu_add_flash(Emulator *emu, uint32_t base, uint32_t size)
{
    ...
    return armv8m_mem_add_ram(&emu->mem, base, size, emu->flash_data);
}
...
int armv8m_emu_add_ram(Emulator *emu, uint32_t base, uint32_t size)
{
    ...
    return armv8m_mem_add_ram(&emu->mem, base, size, emu->ram_data);
}
...
int armv8m_emu_load(Emulator *emu, uint32_t addr, const uint8_t *data, uint32_t size)
{
    ...
    return armv8m_mem_load(&emu->mem, addr, data, size);
}
...
uint32_t armv8m_emu_read_mem(const Emulator *emu, uint32_t addr, uint8_t size, bool *fault)
{
    ...
    /* Debug read bypasses MPU */
    return armv8m_mem_read((MemorySystem *)&emu->mem, addr, size, true, true, fault);
}
...
void armv8m_emu_write_mem(Emulator *emu, uint32_t addr, uint32_t value, uint8_t size, bool *fault)
{
    ...
    /* Debug write bypasses MPU */
    armv8m_mem_write(&emu->mem, addr, value, size, true, true, fault);
}
...
int armv8m_emu_add_peripheral(Emulator *emu, Peripheral *periph, uint32_t base, uint32_t size)
{
    ...
    /* Add MMIO region */
    int result = armv8m_mem_add_mmio(&emu->mem, base, size,
