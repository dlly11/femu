...
TEST(MPUControlExtended, EnableAllFlags)
{
    ...
    CHECK_TRUE(mpu.hfnmiena);
    CHECK_TRUE(mpu.privdefena);
    CHECK_EQUAL(MPU_CTRL_ENABLE | MPU_CTRL_HFNMIENA | MPU_CTRL_PRIVDEFENA, mpu.ctrl);
}
...
TEST(HfnmienaBehavior, HfnmienaBypassAllowsUnmappedAccess)
{
    ...
    allowed = armv8m_mpu_check(&mpu, 0x30000000, 4,
        false, false, true, true, &fault);
    CHECK_TRUE(allowed);
}
...
{
    int result = armv8m_mpu_configure_region(&mpu, -1,
        0x20000000, 0x20000FFF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 0, true);
    CHECK_TRUE(result < 0);
}
...
TEST(RegionConfigExtended, InvalidAttributeIndex)
{
    int result = armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x20000FFF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 10, true);  // attr_idx > 7
    CHECK_TRUE(result < 0);
}
...
TEST(RegionConfigExtended, DisabledRegion)
{
    int result = armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x20000FFF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 0, false);  // Enable = false
    CHECK_EQUAL(ARMV8M_OK, result);
...
TEST(PermissionsExtended, ReadOnlyPrivilegedOnly)
{
    ...
    armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x20000FFF, MPU_AP_RO_PRIV, false,
        MPU_SH_NONE, 0, true);
...
{
    armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x20000FFF, MPU_AP_RW_ALL, true,  // XN
        MPU_SH_NONE, 0, true);
...
TEST(PrivdefenaTests, InstructionFetchInCodeRegion)
{
    ...
    bool allowed = armv8m_mpu_check(&mpu, 0x00001000, 4,
        false, true, true, false, &fault);
    CHECK_TRUE(allowed);
}
...
TEST(BoundaryCrossing, AccessSpanningTwoRegions)
{
    ...
    armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x200000FF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 0, true);

    // Region 1: 0x20000100-0x200001FF, RW
    armv8m_mpu_configure_region(&mpu, 1,
        0x20000100, 0x200001FF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 0, true);

    // Access spanning both regions (starts at 0xFE, size 4 ends at 0x101)
    bool allowed = armv8m_mpu_check(&mpu, 0x200000FE, 4,
        true, false, true, false, &fault);
    CHECK_TRUE(allowed);
}
...
TEST(BoundaryCrossing, AccessSpanningWithMismatchedPermissions)
{
    ...
    armv8m_mpu_configure_region(&mpu, 1,
        0x20000100, 0x200001FF, MPU_AP_RO_ALL, false,
        MPU_SH_NONE, 0, true);
...
TEST(AdditionalCoverage, SingleByteAccess)
{
    ...
    // Size = 1 byte access
    bool allowed = armv8m_mpu_check(&mpu, 0x20000100, 1,
        false, false, true, false, &fault);
    CHECK_TRUE(allowed);
}
...
TEST(AdditionalCoverage, SpanningAccessStartRegionFails)
{
    ...
    armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x200000FF, MPU_AP_RO_ALL, false,
        MPU_SH_NONE, 0, true);
...
TEST(AdditionalCoverage, FetchFaultWithNullFaultPtr)
{
    ...
    // Region with XN
    armv8m_mpu_configure_region(&mpu, 0,
        0x20000000, 0x20000FFF, MPU_AP_RW_ALL, true,
        MPU_SH_NONE, 0, true);
...
TEST(AdditionalCoverage, DataFaultWithFaultPtr)
{
    ...
    bool allowed = armv8m_mpu_check(&mpu, 0x30000000, 1,
        false, false, true, false, &fault);
    CHECK_FALSE(allowed);
    CHECK_EQUAL(MPU_FAULT_DACCVIOL, fault.type);
}
...
TEST(AdditionalCoverage, ZeroSizeAccessDenied)
{
    ...
    bool allowed = armv8m_mpu_check(&mpu, 0x20000100, 0,
        false, false, true, false, &fault);
    CHECK_FALSE(allowed);
    CHECK_EQUAL(MPU_FAULT_DACCVIOL, fault.type);
}
...
TEST(AdditionalCoverage, AddressOverflowDenied)
{
    armv8m_mpu_enable(&mpu, true, false, false);
    armv8m_mpu_configure_region(&mpu, 0,
        0xFFFFFF00, 0xFFFFFFFF, MPU_AP_RW_ALL, false,
        MPU_SH_NONE, 0, true);

    // Access that would overflow: 0xFFFFFFFE + 4 - 1 wraps
    bool allowed = armv8m_mpu_check(&mpu, 0xFFFFFFFE, 4,
        false, false, true, false, &fault);
    CHECK_FALSE(allowed);
}
