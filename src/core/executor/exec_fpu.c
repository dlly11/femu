...
{
    cpu->fpscr &= ~(ARMV8M_FPSCR_N | ARMV8M_FPSCR_Z |
                    ARMV8M_FPSCR_C | ARMV8M_FPSCR_V);
 ...
 */
static bool is_fpu_enabled(const Executor *exec)
{
    return exec->has_fpu;
}
...
static void set_rounding_mode(uint32_t fpscr)
{
    ...
    int mode;
    switch ((fpscr >> 22) & 3) {
        case 0:  mode = FE_TONEAREST;  break;   /* RN - Round to Nearest */
        case 1:  mode = FE_UPWARD;     break;   /* RP - Round towards +infinity */
        case 2:  mode = FE_DOWNWARD;   break;   /* RM - Round towards -infinity */
        case 3:  mode = FE_TOWARDZERO; break;   /* RZ - Round towards Zero */
...
static int check_lazy_preservation(Executor *exec)
{
    ...
    exec->mem.write(exec->mem.ctx, frameptr + 96, cpu->fpscr, ACCESS_WORD, &fault);
    if (fault) {
        cpu->cfsr |= ARMV8M_BFSR_LSPERR;
        return ARMV8M_ERR_BUS_FAULT;
    }
...
int exec_fpu_multi(Executor *exec, const DecodedInsn *insn)
{
    ...
    uint32_t base = get_reg(exec, insn->rn);
    uint32_t count = insn->imm;  /* Number of words (for single) or doublewords (for double) */
    uint32_t regs;
    uint32_t bytes;
    bool fault = false;
        ...
        if (insn->is_double) {
            ...
            if (insn->add || !insn->pre_index) {
                ...
                if (is_load) {
                    ...
                    if (fault) return ARMV8M_ERR_BUS_FAULT;
                    exec->cpu.s[s_base + 1] = mem_read(exec, addr + 4, ACCESS_WORD, &fault);
                    if (fault) return ARMV8M_ERR_BUS_FAULT;
                } else {
                    mem_write(exec, addr, exec->cpu.s[s_base], ACCESS_WORD, &fault);
                    if (fault) return ARMV8M_ERR_BUS_FAULT;
...
int exec_fpu_move(Executor *exec, const DecodedInsn *insn)
{
    ...
    case VFP_VMOV_IMM:
    {
        ...
        if (insn->is_double) {
            /* F64: aBbbbbbb bbcdefgh 00000000 00000000 00000000 00000000 00000000 00000000 */
            uint64_t exp = b ? 0x3FCULL : 0x400ULL;
            exp |= (uint64_t)(b ? 0U : 3U) << 8;
            uint64_t imm64 = ((uint64_t)a << 63) | (exp << 52) | ((uint64_t)bcdefgh << 48);
            union { uint64_t i; double f; } u;
            u.i = imm64;
            ...
            uint32_t imm32 = ((uint32_t)a << 31) | ((uint32_t)exp << 23) | ((uint32_t)bcdefgh << 19);
            union { uint32_t i; float f; } u;
            u.i = imm32;
