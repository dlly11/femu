...
static EmuMemRegion *find_region_internal(EmuMemorySystem *mem, uint64_t addr) {
  for (int i = 0; i < mem->num_regions; i++) {
    ...
    /* Use subtraction to avoid overflow: addr < base + size  =>  addr - base <
     * size */
...
static uint64_t read_ram(const EmuMemRegion *r, uint64_t addr, uint8_t size) {
  ...
  case 1:
    return data[offset];
  case 2:
    return (uint64_t)data[offset] | ((uint64_t)data[offset + 1] << 8);
  case 4:
    return (uint64_t)data[offset] | ((uint64_t)data[offset + 1] << 8) |
 ...
 */
static void write_ram(const EmuMemRegion *r, uint64_t addr, uint64_t value,
                      uint8_t size) {
  uint64_t offset = addr - r->base;
  uint8_t *data = r->data;
 ...
 */
static void report_fault(EmuMemorySystem *mem, uint64_t addr, bool is_write,
                         int fault_type) {
  EMU_LOG_DEBUG(
      EMU_LOG_CAT_MEMORY, "Memory fault: addr=0x%08llX %s fault_type=%d",
...
static void build_page_table(EmuMemorySystem *mem) {
  ...
  for (int i = 0; i < mem->num_regions; i++) {
    ...
    for (uint64_t page = start_page;
         page < end_page && page < mem->page_table_size; page++) {
      mem->page_table[page].region = r;

      /* Pre-compute data base for fast RAM/ROM access */
      if (r->data &&
          (r->type == EMU_MEM_REGION_RAM || r->type == EMU_MEM_REGION_ROM)) {
 ...
 */
static inline EmuMemRegion *fast_find_region(EmuMemorySystem *mem,
                                             uint64_t addr) {
 ...
 */
static inline uint64_t fast_read_ram(const EmuPageEntry *entry, uint64_t addr,
                                     uint8_t size) {
 ...
 */
static inline void fast_write_ram(const EmuPageEntry *entry, uint64_t addr,
...
int emu_mem_add_mmio(EmuMemorySystem *mem, uint64_t base, uint64_t size,
                     void *ctx,
                     uint64_t (*read_cb)(void *ctx, uint64_t offset,
                                         uint8_t size),
                     void (*write_cb)(void *ctx, uint64_t offset,
                                      uint64_t value, uint8_t size)) {
  EmuMemRegion region = {.base = base,
                         .size = size,
...
int emu_mem_init_page_table(EmuMemorySystem *mem, uint64_t max_addr) {
  ...
  uint32_t num_pages =
      (uint32_t)((max_addr + EMU_PAGE_SIZE - 1) >> EMU_PAGE_SHIFT);
