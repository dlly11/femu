...
TEST_GROUP(MemoryInit) {
  ...
  void setup() { emu_mem_init(&mem); }

  void teardown() {}
};

TEST(MemoryInit, InitSetsDefaults) { CHECK_EQUAL(0, mem.num_regions); }
...
static void mock_mmio_write(void *ctx, uint64_t offset, uint64_t value,
                            uint8_t size) {
  (void)ctx;
  (void)size;
...
TEST_GROUP(MMIOOperations) {
  ...
  void setup() {
    emu_mem_init(&mem);
    emu_mem_add_mmio(&mem, 0x40000000, 0x1000, NULL, mock_mmio_read,
                     mock_mmio_write);
...
static void mock_fault_callback(void *ctx, uint64_t addr, bool is_write,
                                int fault_type) {
  (void)ctx;
...
TEST(RegionManagement, AddMaxRegions) {
  ...
  for (int i = 0; i < EMU_MEM_MAX_REGIONS; i++) {
    result = emu_mem_add_ram(&mem, (uint64_t)(0x20000000 + i * 0x1000),
                             sizeof(ram), ram);
    CHECK_EQUAL(EMU_OK, result);
  }
...
static void device_write(void *ctx, uint64_t offset, uint64_t value,
                         uint8_t size) {
  (void)ctx;
  (void)offset;
  (void)size;
