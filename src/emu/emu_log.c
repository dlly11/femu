...
typedef struct {
  ...
  int category_levels[EMU_LOG_CAT_COUNT]; /**< Per-category levels (-1 = use
                                             global) */
...
void emu_log_set_enabled(bool enabled) { g_log_config.enabled = enabled; }
...
void emu_log_impl(int level, int category, const char *file, int line,
  ...
  g_log_config.callback(g_log_config.callback_ctx, level, category, filename,
                        line, func, msg);
}
